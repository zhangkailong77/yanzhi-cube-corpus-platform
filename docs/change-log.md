...
- 文件: `README.md`
- 变更:
  - 将原模板型 README 全量重写为内部交接主手册。
  - 新增项目概述、架构说明、目录结构、环境变量、开发调试、数据库初始化、Docker 部署、鉴权说明、音频与导入逻辑、常见操作、故障排查、已知风险、接手建议等章节。
  - 明确记录当前生产环境地址、端口、默认管理员账号、脚本硬编码风险、voicedatas 与 media 目录关系等关键交接信息。

- 文件: `docs/change-log.md`
- 变更:
  - 追加本次 README 重写的变更记录，满足仓库要求。
